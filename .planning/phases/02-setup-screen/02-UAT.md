...
number: 1
name: WR-02 double-pop race (manual back tap vs. auto-pop-on-done)
expected: |
  On PlaceholderRunningScreen, let the timer reach TimerPhase.done (or use a very short duration)
  and tap the back arrow at the exact moment auto-return would fire; repeat several times rapidly.
  The app returns to Setup exactly once per attempt — no crash, no Navigator assertion, no visible
  double-navigation flicker.
